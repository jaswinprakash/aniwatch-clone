...
const SearchPage = () => {
  ...
  const getHomeList = async () => {
    /* Filter/Genre fetching commented out for now
      ...
      debounce(async (query, genre, category) => {
        ...
        if (query) {
          /* Filter logic commented out
                    ...
                    */
          try {
            // Older API call: const response = await apiConfig.get(`/api/v2/hianime/search?q=${query}&type=${category}&genres=${formattedGenres}`);
            const response = await apiConfig.get(`/search?q=${query}`);
